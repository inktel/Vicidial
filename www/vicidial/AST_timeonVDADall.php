...
if (!isset($orderby))		{$orderby='statusdown';}
...
$HTbarge =			" BARGE |";
$HDstatus =			"----------+";
$HTstatus =		" <a href=\"$PHP_SELF?$groupQS&RR=$RR&DB=$DB&adastats=$adastats&SIPmonitorLINK=$SIPmonitorLINK&IAXmonitorLINK=$IAXmonitorLINK&usergroup=$usergroup&UGdisplay=$UGdisplay&UidORname=$UidORname&orderby=$groupord&SERVdisplay=$SERVdisplay&CALLSdisplay=$CALLSdisplay&PHONEdisplay=$PHONEdisplay&CUSTPHONEdisplay=$CUSTPHONEdisplay&with_inbound=$with_inbound&monitor_active=$monitor_active&monitor_phone=$monitor_phone&ALLINGROUPstats=$ALLINGROUPstats&DROPINGROUPstats=$DROPINGROUPstats&NOLEADSalert=$NOLEADSalert&CARRIERstats=$CARRIERstats&PRESETstats=$PRESETstats&AGENTtimeSTATS=$AGENTtimeSTATS\">STATUS</a>   |";
if ($RTajax > 0)
	{$HTstatus =	" <a href=\"#\" onclick=\"update_variables('orderby','status');\">STATUS</a>   |";}
...
if ($orderby=='statusup') {$orderSQL='vicidial_live_agents.status,last_call_time';}
if ($orderby=='statusdown') {$orderSQL='vicidial_live_agents.status desc,last_call_time desc';}
	...
	#		if ($call_time_S >= 600) {$G='<SPAN class="purple"><B>'; $EG='</B></SPAN>';}
		...
		if ($Lstatus=='PAUSED') 
			...
			if (!ereg("N",$agent_pause_codes_active))
				...
				$stmtC="select left(pause_code_name,10) from vicidial_pause_codes where pause_code='$pausecode' and campaign_id = '$campaign_id';";
				$rsltC=mysql_query($stmtC,$link);
				$pause_code_record = mysql_num_rows($rslt);
				if ($pause_code_record > 0)
					{
					$rowC=mysql_fetch_row($rsltC);
					$pausecode = sprintf("%-6s", $rowC[0]);
					$pausecode = substr("$pausecode               ",0,15);
					}
				else
					{
					$pausecode = "$pausecode         ";
...
#				{$G='<SPAN class="orange"><B>'; $EG='</B></SPAN>';}
		...
		if ( (eregi("DEAD",$status))) {$status = ' WRAP ';}

		if ($realtime_block_user_info > 0)
			{
			$Aecho .= "|$UGD $G$sessionid$EG$L$R$Aring_note[$i]| $G$status$EG $CM $G$pausecode$EG|$CP$SVD$G$call_time_MS$EG | $G$campaign_id$EG | $G$calls_today$EG |$INGRP\n";
			}
		if ($realtime_block_user_info < 1)
			{
			$Aecho .= "| $G$extension$EG$Aring_note[$i]|$phoneD<a href=\"./user_status.php?user=$Luser\" target=\"_blank\">$G$user$EG</a> <a href=\"javascript:ingroup_info('$Luser','$j');\">+</a> |$UGD $G$sessionid$EG$L$R | $G$status$EG $CM $G$pausecode$EG|$CP$SVD$G$call_time_MS$EG | $G$campaign_id$EG | $G$calls_today$EG |$INGRP\n";
			}
		$j++;
		}
